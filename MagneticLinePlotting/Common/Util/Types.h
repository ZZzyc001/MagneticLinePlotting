...
namespace PhysX {
    ...
    namespace detail {
        ...
        struct UpgradeImpl {
            using DrvType = Eigen::Matrix<Type, Dim, 1>;
        };
        template<typename Type, int Rows, int Dim>
        struct UpgradeImpl<Eigen::Matrix<Type, Rows, 1>, Dim> {
            using DrvType = Eigen::Matrix<Type, Rows, Dim>;
        };
        template<typename Type>
        struct DowngradeImpl;
        template<typename Type, int Dim>
        struct DowngradeImpl<Eigen::Matrix<Type, Dim, 1>> {
            using IntType = Type;
        };
        template<typename Type, int Rows, int Dim>
            requires(Dim > 1)
        struct DowngradeImpl<Eigen::Matrix<Type, Rows, Dim>> {
            using IntType = Eigen::Matrix<Type, Rows, 1>;
        };
    }
    template<typename Type, int Dim>
    using Upgrade = typename detail::UpgradeImpl<Type, Dim>::DrvType;
    template<typename Type>
    using Downgrade = typename detail::DowngradeImpl<Type>::IntType;
...
#undef DECLARE_EIGEN_MATRIX_TYPES
    ...
    using Array = Eigen::Array<Scalar, Dim, 1, Eigen::ColMajor, Dim, 1>;
    template<int Dim, typename Scalar>
    using Vector = Eigen::Matrix<Scalar, Dim, 1, Eigen::ColMajor, Dim, 1>;
    template<int Dim, typename Scalar>
    using Matrix = Eigen::Matrix<Scalar, Dim, Dim, Eigen::ColMajor, Dim, Dim>;
