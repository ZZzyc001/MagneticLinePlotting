...
namespace PhysX::IO {
    ...
    template<typename Type>
    inline void readArray(std::istream & in, Type * const data, const size_t cnt) {
        if (cnt > 0) read(in, data, sizeof(Type) * cnt);
    }
    ...
    template<typename Type>
    inline void writeArray(std::ostream & out, const Type * const data, const size_t cnt) {
        if (cnt > 0) write(out, data, sizeof(Type) * cnt);
    }
